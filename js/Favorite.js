...
export class Favorites {
  ...
  checkUserLength() {
    if (this.entries.length === 0) {
      document.querySelector(".state").style.display = ""
    } else {
...
export class FavoritesView extends Favorites {
  ...
  update() {
    ...
    this.entries.forEach((user) => {
      ...
      row.querySelector(".btn-remove").addEventListener("click", () => {
        const isOk = confirm('Remover este usuário ?')
        if(isOk) {
          this.delete(user)
        }
      })
  ...
  createRow() {
    ...
    tr.innerHTML = `
    <td class="user">
    <img src="https://github.com/xumbreks.png" />
    <a href="https://github.com/xumbreks" target="_blank">
      <p>Brian</p>
      <span> /xumbreks </span>
    </a>
    </td>
